...
impl GameBoard {
    ...
    pub fn resolve_horizontal_matches(
        ...
        for y in 0..self.dimensions.y {
            ...
            let mut color_to_match = tile_query
                .get_component::<TileDesc>(self.forward[self.idx((0, y).into())].unwrap())
                .unwrap()
                .color;

            for x in 1..self.dimensions.x {
                let next_entity_color = tile_query
                    .get_component::<TileDesc>(self.forward[self.idx((x, y).into())].unwrap())
                    .unwrap()
                    .color;
                if next_entity_color == color_to_match {
                    ...
                    if match_counter >= MIN_MATCH_LENGTH {
                        ...
                        for backtrace in first_match..x {
                            to_be_deleted
                                .insert(self.forward[self.idx((backtrace, y).into())].unwrap());
    ...
    pub fn resolve_vertical_matches(
        ...
        for x in 0..self.dimensions.x {
            let mut match_counter: u32 = 1;
            let mut color_to_match = tile_query
                .get_component::<TileDesc>(self.forward[self.idx((x, 0).into())].unwrap())
                .unwrap()
                .color;
                ...
                if next_entity_color == color_to_match {
                    ...
                    if match_counter >= MIN_MATCH_LENGTH {
                        ...
                        for backtrace in first_match..y {
                            to_be_deleted
                                .insert(self.forward[self.idx((x, backtrace).into())].unwrap());
